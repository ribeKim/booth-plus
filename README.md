...
| `dev` | `https://dev.booth-plus.ribe.moe/api` | `bun run dev:extension:dev` | `bun run build:extension:dev` |
| `prod` | `https://booth-plus.ribe.moe/api` | `bun run dev:extension:prod` | `bun run build:extension:prod` |

기본 `bun run dev:extension`은 `local`, 기본 `bun run build:extension`과 루트 `bun run build`는 `prod`를 사용합니다. 환경별 주소는 `apps/extension/package.json`의 mode 스크립트에서 `WXT_API_ORIGIN`으로 주입합니다. 백엔드는 Discord 인증, 사용자 설정, 상품 조회, 리뷰 CRUD와 투표 API를 제공합니다.
