...
확장 프로그램은 WXT mode에 따라 API 서버와 manifest host permission을 함께 선택합니다.

| mode | API | 개발 실행 | 빌드 |
| --- | --- | --- | --- |
| `local` | `http://localhost:3000/api` | `bun run dev:extension:local` | `bun run build:extension:local` |
| `dev` | `https://booth-plus.ribe.moe/api` | `bun run dev:extension:dev` | `bun run build:extension:dev` |
| `prod` | `https://vbt.kamyu.me/api` | `bun run dev:extension:prod` | `bun run build:extension:prod` |

기본 `bun run dev:extension`은 `local`, 기본 `bun run build:extension`과 루트 `bun run build`는 `prod`를 사용합니다. 환경별 주소는 `apps/extension/package.json`의 mode 스크립트에서 `WXT_API_ORIGIN`으로 주입합니다. dev 백엔드는 아직 상태 확인과 PostgreSQL 기반만 구현한 단계이므로 `/auth`, `/user`, `/product`, `/comment` 구현과 전환 테스트 전에는 prod 주소를 dev 서버로 바꾸면 안 됩니다.
