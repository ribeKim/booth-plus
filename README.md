...
│  └─ backend/     # Bun + Hono + PostgreSQL API
...
Bun 1.3.1 이상, Node.js 22 이상, Docker와 Docker Compose가 필요합니다.
...
| `bun run test:integration` | 실제 PostgreSQL을 사용하는 백엔드 통합 테스트 실행 |
| `bun run build` | 모든 패키지 빌드 |
| `bun run db:migrate` | 현재 `DATABASE_URL`의 PostgreSQL에 미적용 마이그레이션 적용 |
| `bun run db:migration:create -- <name>` | 새 PostgreSQL SQL 마이그레이션 생성 |
